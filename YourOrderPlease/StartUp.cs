namespace YourOrderPlease
{
    using System;
    using System.Linq;

    public static class StartUp
    {
        public static void Main(string[] args)
        {
            Console.WriteLine(Order(""));
        }

        /*Your task is to sort a given string. Each word in the String will contain a single number. 
         *This number is the position the word should have in the result.

        Note: Numbers can be from 1 to 9. So 1 will be the first word (not 0
        If the input String is empty, return an empty String. The words in the input String will only contain valid consecutive numbers
        For an input: "is2 Thi1s T4est 3a" the function should return "Thi1s is2 3a T4est"*/

        // shortest/best solution
        //            if (string.IsNullOrEmpty(words)) return words;
        //           return string.Join(" ", words.Split(' ').OrderBy(s => s.ToList().Find(c => char.IsDigit(c))));

        public static string Order(string words)
        {
            if (words.Length != 0)
            {
                var parsedWords = words.Split();
                var orderedWords = new string[parsedWords.Length];

                for (int i = 0; i < parsedWords.Length; i++)
                {
                    var position = int.Parse(parsedWords[i].First(c => Char.IsDigit(c)).ToString());
                    orderedWords[position - 1] = parsedWords[i];
                }

                return String.Join(" ", orderedWords);
            }

            return "";
        }
    }
}
